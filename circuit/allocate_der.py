...
def create_der_list (cfg_file, froot, mRID, outpath, pv_size, pv_penetration):
  ...
  print ('PV_1,node_60,ABC,Photovoltaic,2230.0,4.16,2000.0,0.0', file=op)
  print ('Bat_PV,node_60,ABC,Battery,2000.0,4.16,0.0,0.0,8000.0,4000.0', file=op)
  print ('WTG_1,node_1,ABC,SynchronousMachine,2230.0,4.16,2000.0,0.0', file=op)
  print ('Bat_WTG,node_1,ABC,Battery,2000.0,4.16,0.0,0.0,8000.0,4000.0', file=op)
  print ('// adding rooftop PV sized {:.2f} kW at {:.2f}% penetration'.format (kw, 100.0 * pv_penetration), file=op)
...
#  print ('\nEnergyConsumer binding keys are:',ret.variables)
  for b in ret.bindings:
    ...
    if np.random.uniform (0, 1) <= pv_penetration:
      numpv += 1
      print ('Rooftop_{:d},{:s},s1s2,Photovoltaic,{:.3f},{:.3f},{:.3f},{:.3f}'.format (numpv, b['bus'].value, kva, kv, kw, kvar), file=op)

  op.close()
  print ('added {:d} PV sized {:.2f} kW for {:.2f}% penetration'.format (numpv, kw, 100.0 * pv_penetration))
